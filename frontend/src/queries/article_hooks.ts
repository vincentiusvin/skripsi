...
export function useArticlesDetailGet(opts: {
    ...
    return useQuery({
      ...
      queryFn: () =>
        new APIContext("ArticlesDetailGet").fetch(`/api/articles/${article_id}`), // Fetch contribution detail by ID
      retry, // Retry logic
    });
  }
  ...
  export function useArticlesDetailPut(opts: {
    article_id: number;
    onSuccess?: () => void;
  }) {
    const { article_id, onSuccess } = opts;
    return useMutation({
      mutationFn: new APIContext("ArticlesDetailPut").bodyFetch(
